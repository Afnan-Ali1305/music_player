...
final songsProvider = StateNotifierProvider<SongsNotifier, SongsState>((ref) {
  return SongsNotifier(ref);   // Pass ref so we can watch other providers if needed
});
...
class SongsNotifier extends StateNotifier<SongsState> {
  ...
  Future<void> _init() async {
    // Background playback callbacks (Next / Previous / Song Complete)
    audioHandler.onSkipToNext = () => nextSong();
    audioHandler.onSkipToPrevious = () => previousSong();
    ...
    // Listen to position & duration for UI
    audioHandler.positionStream.listen((position) {
      state = state.copyWith(currentSongPosition: position);
    });
    ...
    // IMPORTANT: Do NOT call loadFavourites() or fetchAlbums() here anymore
    // They are now handled by their own providers
  }
  ...
  Future<void> fetchAllSongs() async {
    ...
    final rawSongs = await audioQuery.querySongs(
      sortType: SongSortType.DATE_ADDED,
      orderType: OrderType.DESC_OR_GREATER,
      uriType: UriType.EXTERNAL,
      ignoreCase: true,
    ...
    final filteredSongs = rawSongs.where((song) {
      final path = song.data.toLowerCase();
      return song.isMusic == true &&
          (song.duration ?? 0) > 120000 && // > 2 minutes
          !path.contains("whatsapp") &&
          !path.contains("record");
    }).toList();

    final songsList = filteredSongs
        .map((s) => Song(
              songID: s.id,
              songName: s.title,
              songArtist: s.artist ?? "Unknown Artist",
    ...
    final durations = filteredSongs
        .map((s) => Duration(milliseconds: s.duration ?? 0))
        .toList();

    // Restore recently played from storage
    final recentIds = LocalStorage.getRecentlyPlayedIds();
    ...
    state = state.copyWith(
      allSongs: songsList,
      recentlyPlayedSongs: recentSongs,
      currentQueue: songsList,
      isLoading: false,
      hasPermission: true,
      allSongsDuration: durations,
    );
  }

  // ================= PLAYBACK =================
  Future<Uri?> _getArtworkUri(Uint8List? artworkBytes, int songId) async {
    if (artworkBytes == null) return null;
    final tempDir = await getTemporaryDirectory();
    final file = File('${tempDir.path}/artwork_$songId.jpg');
    await file.writeAsBytes(artworkBytes);
  ...
  Future<void> _playViaHandler(Song song) async {
    ...
    final artUri = await _getArtworkUri(artworkBytes, song.songID);
  ...
  void _addToRecentlyPlayed(Song song) {
    ...
    currentRecent.removeWhere((s) => s.songID == song.songID);
    currentRecent.insert(0, song);
    if (currentRecent.length > 10) currentRecent.removeLast();
  ...
  // ================= GETTERS =================
  List<Song> get allSongs => state.allSongs;
}
